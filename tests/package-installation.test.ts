...
const packageName = 'typeorm-transactional-tests';
const tarballExtension = '.tgz';
const projectPrefix = 'typeorm-transactional-tests-project-';
const tarballPrefix = 'typeorm-transactional-tests-tarball-';
...
describe(suiteTitle, () => {
  it('packs and installs successfully in a clean project', () => {
    ...
    try {
      execFileSync(pnpmCommand, ['pack', packDestinationFlag, tarballDirectory], { stdio: 'inherit' });
      const tarballName = readdirSync(tarballDirectory).find((file) => file.endsWith(tarballExtension));
      if (tarballName === undefined) {
        throw new Error(tarballErrorMessage);
      }
      const tarballPath = join(tarballDirectory, tarballName);

      const initialPackageJson = { name: 'test-installation', version: '1.0.0' };
      writeFileSync(join(projectDirectory, packageJsonFile), JSON.stringify(initialPackageJson, null, 2));
      execFileSync(pnpmCommand, ['add', '--save-dev', tarballPath], { cwd: projectDirectory, stdio: 'inherit' });
      ...
      const devDependencies = (packageJson[dependencySection] as Record<string, string> | undefined) ?? {};
