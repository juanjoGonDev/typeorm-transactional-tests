...
const createRandomCategories = (seed: string): CategorySeed[] => {
  const selections = pickUniqueValues(
    categoryPool,
    randomEntityConfig.categories,
    `${seed}-categories`
  );
...
const createRandomUsers = (seed: string): UserSeed[] => {
  ...
  for (let index = 0; index < randomEntityConfig.users; index += 1) {
    ...
    const addresses = Array.from(
      { length: randomEntityConfig.addressesPerUser },
      (_, addressIndex) => {
        return {
...
const createRandomProducts = (
  seed: string,
  categories: ReadonlyArray<CategorySeed>
): ProductSeed[] => {
  ...
  for (let index = 0; index < randomEntityConfig.products; index += 1) {
    ...
    const priceValue =
      random.nextInt(numericRanges.price.min, numericRanges.price.max) +
      random.nextFloat();
    const inventoryCount = random.nextInt(
      numericRanges.inventory.min,
      numericRanges.inventory.max
    );
...
export const buildSeedDataset = (seed: string): SeedDataset => {
  ...
  const products = [
    ...fixedProducts,
    ...createRandomProducts(seed, categories),
  ];
