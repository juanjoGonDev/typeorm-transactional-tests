...
import type { EntityDefinitionList } from "../entities/entity-definition";
...
const postgresDatabaseExistsQuery =
...
type MysqlLikeEnv =
  | typeof environmentVariableNames.mysql
  | typeof environmentVariableNames.mariadb;
...
const normalizeDriver = (value: string | undefined): SupportedDriver => {
  ...
  const driver = supportedDriverTypes.find(
...
const deriveWorkerAwareFile = (filePath: string): string => {
  ...
  return join(
    directory,
    extension === "" ? workerAwareName : `${workerAwareName}${extension}`
  );
};
...
const buildMysqlLikeOptions = (
  ...
  const database = buildWorkerAwareName(
    env[envNames.database] ?? defaults.database
  );
  return {
    type,
...
const buildPostgresOptions = (entities: EntityCollection): PostgresOptions => {
  ...
  const database = buildWorkerAwareName(
    env[names.database] ?? postgresDefaults.database
  );
  return {
    type: "postgres",
...
const buildSqliteOptions = (entities: EntityCollection): DataSourceOptions => {
  ...
  return {
    type: "sqlite",
    database:
      env[environmentVariableNames.sqlite.database] ?? sqliteDefaults.database,
    entities: toEntityList(entities),
    ...schemaSynchronization,
  };
};

const buildBetterSqliteOptions = (
  entities: EntityCollection
): DataSourceOptions => {
  const env = process.env;
  const configuredPath =
    env[environmentVariableNames.betterSqlite.database] ??
    sqliteDefaults.database;
...
const createMysqlPreparation = (
  ...
  return async () => {
    const env = process.env;
    const adminCredentials = resolveMysqlAdminCredentials(
      envNames,
      defaults,
      env
    );
    ...
    try {
      await pool.query(
        `${mysqlCreateDatabaseStatement} ${formatMysqlIdentifier(
          options.database
        )}`
      );
      await pool.query(
        `${mysqlGrantPrivilegesStatement} ${formatMysqlIdentifier(
          options.database
        )}.* ${mysqlToClause} ${formatMysqlUserReference(
          options.username,
          runtimeHost
        )}`
      );
...
const createPostgresPreparation = (
  ...
  return async () => {
    ...
    try {
      ...
      const databaseExists = await client.query(postgresDatabaseExistsQuery, [
        options.database,
      ]);
      if (databaseExists.rowCount === 0) {
        await client.query(
          `${postgresCreateDatabaseStatement} ${formatPostgresIdentifier(
            options.database
          )}`
        );
      }
...
const createBetterSqlitePreparation = (
  databasePath: string
): DatabasePreparation => {
...
const createTestDatabaseConfiguration = (
  entities: EntityCollection
): TestDatabaseConfiguration => {
  const driver = normalizeDriver(process.env[environmentVariableNames.type]);

  if (driver === driverNames.mysql) {
    const options = buildMysqlLikeOptions(
      entities,
      mysqlDefaults,
      environmentVariableNames.mysql,
      driverNames.mysql
    );
    return {
      options,
      prepare: createMysqlPreparation(
        options,
        environmentVariableNames.mysql,
        mysqlDefaults
      ),
  ...
  if (driver === driverNames.mariadb) {
    const options = buildMysqlLikeOptions(
      entities,
      mariadbDefaults,
      environmentVariableNames.mariadb,
      driverNames.mariadb
    );
    return {
      options,
  ...
  if (driver === driverNames.betterSqlite) {
    ...
    return {
      options,
      prepare: createBetterSqlitePreparation(options.database as string),
    };
  }
...
export const resolveTestDatabaseConfiguration = (
  entities: EntityCollection
): TestDatabaseConfiguration => {
  return createTestDatabaseConfiguration(entities);
};

export const resolveTestDataSourceOptions = (
  entities: EntityCollection
): DataSourceOptions => {
