...
const persistCategories = async (manager: EntityManager, dataset: SeedDataset): Promise<Map<string, Category>> => {
  const repository = manager.getRepository(Category);
  const entities = dataset.categories.map((category) => repository.create(category));
...
const persistUsers = async (manager: EntityManager, dataset: SeedDataset): Promise<Map<string, User>> => {
  ...
  const entities = dataset.users.map((user) => {
    return repository.create({
      ...
      addresses: user.addresses.map((address) => manager.create(Address, address))
    });
  });
  const saved = await repository.save(entities);
...
export const seedDatabase = async (manager: EntityManager): Promise<SeedSummary> => {
